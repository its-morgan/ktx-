...
<div class="linear-shell">
    ...
    <main class="mx-auto w-full max-w-6xl animate-fade-up p-4 pb-24 sm:p-6 lg:pb-6">
        <x-breadcrumbs />
    ...
    <nav class="linear-navbar-glass fixed bottom-0 left-0 right-0 z-50 border-t p-2 lg:hidden">
        <div class="mx-auto flex w-full max-w-lg items-center justify-between">
            ...
            <a href="{{ route('student.hoadoncuaem') }}" class="flex flex-col items-center gap-0.5 rounded-md px-3 py-1 text-xs {{ request()->routeIs('student.hoadoncuaem') ? 'text-slate-900' : 'text-slate-500' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
